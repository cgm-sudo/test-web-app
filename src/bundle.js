...
webix.ui({
  rows: [{}, {
    cols: [{}, {
      view: "button",
      id: "main_btn",
      label: "<i class='webix_icon wxi-sync' style='font-size:30px;'></i>" + "<h1 style='margin:0'>Unit Converter</h1>",
...
function viewGen(item1, item2, tool_desc, given_id) {
  var view_out = {
    view: "button",
    id: given_id,
    label: `<h3 style='margin:0'>${item1}</h3>` + `<i class='webix_icon wxi-sync' style='font-size:30px;'></i>` + `<h3 style='margin:0'>${item2}</h3>`,
    css: "webix_primary",
...
webix.attachEvent("onClick", function () {
  ...
  if (inp_label === "Mass") {
    ...
    $$("mv_out_field").setValue(vol2mass(inp_val, input_selected, output_selected, density_val, density_selected));
  }
});

},{"./mv_convert":45}],45:[function(require,module,exports){
    ...
    rows: [{
      ...
      cells: [{
        ...
        rows: [{
          ...
          cols: [{}, mv_density_label, mv_density_field, mv_density_select, {}]
        }, {
          height: 20
        }, {
          cols: [{}, mv_convert_button, {}]
        }]
      }, {
        id: "gas_view",
        template: "test2-gas"
...
},{"./units":46,"convert-units":24}],46:[function(require,module,exports){
var mass_flow_SI = {
  'id': 'kg/s',
  'up1': 'kg',
  'bt1': 's'
